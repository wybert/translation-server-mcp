...
const DEFAULT_BASE_URL = process.env.ZOTERO_WEB_API_URL || "https://api.zotero.org";
...
async function save(items, options) {
	const apiKey = process.env.ZOTERO_WEB_API_KEY;
	if (!apiKey) {
		throw new Error("ZOTERO_WEB_API_KEY is required for Zotero Web API");
	}
	const libraryType = (options && options.libraryType) || process.env.ZOTERO_WEB_LIBRARY_TYPE;
	const libraryId = (options && options.libraryId) || process.env.ZOTERO_WEB_LIBRARY_ID;
	if (!libraryType || !libraryId) {
		throw new Error("ZOTERO_WEB_LIBRARY_TYPE and ZOTERO_WEB_LIBRARY_ID are required for Zotero Web API");
	}
	...
	if (response.statusCode >= 400) {
		...
		throw new Error(`zotero web save failed (${response.statusCode}): ${detail}`);
	}
	return {
